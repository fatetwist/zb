...
def get_tasks_async_2(n):
    while True:
        try:
            ...
            res_content = requests.request('GET', spec_url, headers=user.ultimate_headers,
                                           cookies=user.ultimate_cookies).content
    ...
    if t != 'yes':
        ...
        if mode == '1':
            ...
            try:
                ...
                print('[成功] 系统已经接受范围信息(已记住,如果重新输入请修改目录文件key_info.cfg)：单价%s~%s,数量%s~%s,总价%s~%s' % (
                    s_price_round[0], s_price_round[1], counts_round[0], counts_round[1], price_round[0],
                    price_round[1]))
            except:
            ...
            try:
                ...
                price_round = [float(x) for x in price_round.split('-')]
                print('[成功] 系统已经接受范围信息(已记住,如果重新输入请修改目录文件key_info.cfg)：单价%s~%s,数量%s~%s,总价%s~%s' % (
                s_price_round[0], s_price_round[1], counts_round[0], counts_round[1], price_round[0], price_round[1]))
            except:
                print('[警告] 格式输入错误! 3秒后退出程序!')
                time.sleep(3)
                exit(0)
